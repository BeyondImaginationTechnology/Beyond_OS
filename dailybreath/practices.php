...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Faith Practices | DailyBreath</title><?=dailybreath_web_head('Faith Practices | DailyBreath')?><style>
...
<section class="card breath-card" id="breath-practice" data-inhale="<?= (int)$exercise['inhale_seconds'] ?>" data-hold="<?= (int)$exercise['hold_seconds'] ?>" data-exhale="<?= (int)$exercise['exhale_seconds'] ?>" data-default-cycles="<?= (int)$exercise['cycles'] ?>">
  ...
  <div class="breath-settings"><div><label for="cycle-select">Session length</label><select id="cycle-select"><option value="2">2 cycles · quick reset</option><option value="5">5 cycles · calm focus</option><option value="10">10 cycles · extended prayer</option></select></div><button class="btn secondary" id="sound-toggle" type="button" aria-pressed="true">🔔 Gentle cues</button></div>
...
(()=>{
 ...
 const start=document.getElementById('breath-start'), pause=document.getElementById('breath-pause'), reset=document.getElementById('breath-reset'), select=document.getElementById('cycle-select'), sound=document.getElementById('sound-toggle'), complete=document.getElementById('breath-complete'), focus=document.getElementById('breath-focus'), focusExit=document.getElementById('breath-focus-exit');

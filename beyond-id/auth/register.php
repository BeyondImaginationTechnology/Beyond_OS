...
</style></head><body><main class="page"><section class="story"><div class="logo">BEYOND OS · BETA</div><h1>One account.<span>A universe of apps.</span></h1><p>Create your Beyond ID, verify your email, and carry your profile and bit$ wallet through every experience.</p></section><section class="card"><h2>Create Beyond ID</h2><p class="sub">Your secure key to the ecosystem.</p><?php if($error): ?><div class="error"><?= e($error) ?></div><?php endif; ?><?php if($success): ?><div class="success"><?= e($success) ?></div><?php endif; ?><?php if(!$success): ?><div class="social"><?php foreach ($providers as $provider => [$label, $icon]): ?><?php if (beyond_social_enabled($provider)): ?><a class="<?= e($provider) ?>" href="oauth-start.php?provider=<?= e($provider) ?>"><?= e($icon) ?>&nbsp; Sign up with <?= e($label) ?></a><?php else: ?><span class="disabled" aria-disabled="true"><?= e($icon) ?>&nbsp; Sign up with <?= e($label) ?></span><?php endif; ?><?php endforeach; ?></div><?php if (array_filter(array_keys($providers), static fn(string $provider): bool => !beyond_social_enabled($provider))): ?><p class="social-note">Unavailable providers will activate when their client ID and secret are configured.</p><?php endif; ?><div class="divider">or create with email</div><form method="post"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><div class="grid"><div class="field"><label>First name</label><input name="first_name" value="<?= e($_POST['first_name'] ?? '') ?>" required></div><div class="field"><label>Last name</label><input name="last_name" value="<?= e($_POST['last_name'] ?? '') ?>" required></div></div><div class="field"><label>Email address</label><input type="email" name="email" value="<?= e($_POST['email'] ?? '') ?>" required></div><div class="field"><label>Password</label><input type="password" name="password" minlength="8" required></div><div class="field"><label>Confirm password</label><input type="password" name="confirm_password" minlength="8" required></div><label class="check"><input type="checkbox" name="accept_terms" value="1" required><span>I agree to the <a href="terms.php" target="_blank">Terms and Conditions</a> and acknowledge the <a href="privacy.php" target="_blank">Privacy Policy</a>.</span></label><button>Create account and verify email →</button></form><?php endif; ?><p class="login"><a href="login.php">Already have a Beyond ID? Sign in</a></p></section></main><script src="/assets/js/visitor-analytics.js" defer></script></body></html>

...
body.app-shell .translation{border:1px solid var(--language-border);box-shadow:inset 0 3px 0 var(--language-accent)}body.app-shell .translation small{color:var(--language-accent);font-weight:900}body.app-shell .translation-fr{--language-accent:#70aaff;--language-border:rgba(23,104,255,.52);background:rgba(23,104,255,.23)}body.app-shell .translation-ht{--language-accent:#ff7187;--language-border:rgba(228,71,97,.52);background:rgba(228,71,97,.23)}body.app-shell .translation-jm{--language-accent:#43d49a;--language-border:rgba(7,155,105,.52);background:rgba(7,155,105,.23)}body.app-shell .translation-es{--language-accent:#ffc365;--language-border:rgba(196,123,24,.52);background:rgba(196,123,24,.23)}
...
body.app-shell .language-set-control{display:grid;grid-template-columns:auto minmax(0,1fr);gap:6px 10px;align-items:center;max-width:720px;margin:0 auto 10px;padding:0 20px;color:var(--muted);font-size:.78rem}body.app-shell .language-set-control label{font-weight:900;color:var(--text)}body.app-shell .language-set-control select{min-width:0;border:1px solid #ffd24d;background:#ffbf00;color:#07152f;border-radius:12px;padding:9px 11px;font-size:.78rem;font-weight:800}body.app-shell .language-set-control small{grid-column:2}@media(max-width:560px){body.app-shell .language-set-control{padding:0 18px;grid-template-columns:1fr}.language-set-control small{grid-column:auto}}
body.app-shell .today-stats{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}body.app-shell .today-stats span{padding:8px 10px;border:1px solid #ffd24d;border-radius:999px;background:#ffbf00;color:#07152f;font-size:.76rem;font-weight:800}body.app-shell .today-stats strong{color:#07152f}
...
<script>(function(){const frenchSplash=document.getElementById('french-splash'),savedDifficulty=localStorage.getItem('beyond-french.difficulty'),cast=document.getElementById('difficulty-cast'),castBase='<?= h($frenchBase) ?>assets/images/tutors/';const castByAge={kids:[['Louis','louis.jpg'],['Irie','irie.jpg'],['Jazzy','jazzy.jpg'],['Pablo','pablo.jpg']],teen:[['Louis','louis.jpg'],['Irie','irie.jpg'],['Jazzy','jazzy.jpg'],['Pablo','pablo.jpg']],adult:[['Louis','louis.jpg'],['Irie','irie.jpg'],['Jazzy','jazzy.jpg'],['Pablo','pablo.jpg']]};function choose(button){const age=button.dataset.age,difficulty=button.dataset.difficulty;localStorage.setItem('beyond-french.difficulty',difficulty);localStorage.setItem('beyond-french.age',age);cast.innerHTML=castByAge[age].map((t)=>'<figure><img src="'+castBase+(age==='kids'?'':age==='teen'?'high-school/':'advanced/')+t[1]+'" alt="'+t[0]+'"><figcaption>'+t[0]+'</figcaption></figure>').join('');document.querySelectorAll('[data-splash-tutor]').forEach(image=>{image.src=castBase+(age==='kids'?'':age==='teen'?'high-school/':'advanced/')+image.dataset.splashTutor+'.jpg'});cast.classList.add('visible');sessionStorage.setItem('beyond-french-entered-v2','1');frenchSplash.classList.add('hidden')}document.querySelectorAll('[data-difficulty]').forEach(button=>button.addEventListener('click',()=>choose(button)));if(sessionStorage.getItem('beyond-french-entered-v2')==='1'&&savedDifficulty)frenchSplash.classList.add('hidden');})();</script>
...
<section class="section" id="today">
    ...
    <article class="lesson-card">
        ...
        <div class="lesson-next">
            ...
            <a class="button daily-challenge-button" href="challenge.php?id=<?= (int)$lesson['id'] ?>">Start daily challenge <span aria-hidden="true">→</span></a>
        </div>
    </article>
</section>
<?php endif; ?>

...
$jaguarPrompt = $tradition === 'quran'
    ? 'Ask Moe a respectful question about the Quran.'
    : ($tradition === 'torah' ? 'Ask Dovi a respectful question about the Tanakh.' : 'Ask Chris a question about the Bible.');
$jaguarHref = 'https://ai.beyondimagination.co.technology/chat.php?guide=' . rawurlencode(strtolower($jaguarGuide)) . '&prompt=' . rawurlencode($jaguarPrompt);
...
$verses = dailybreath_sacred_chapter($tradition, $book, $chapter, $locale);
$query = substr(trim((string)($_GET['q'] ?? '')), 0, 100);
$searchResults = $query === '' ? [] : dailybreath_search_sacred_text($tradition, $query, 75, $locale);

$bookName = $tradition === 'quran'
    ? ($surahs[(int)$book] ?? ('Surah '.$book))
    : ($tradition === 'torah' ? dailybreath_jewish_book_name($book) : dailybreath_bible_book_name($book, $locale));
...
$translation = $tradition === 'torah'
    ? ($locale === 'fr' ? 'Tanakh en français, traduction Louis Segond 1910, domaine public. Les noms des livres suivent l’usage juif.'
        : ($locale === 'es' ? 'Tanaj en español, Reina-Valera 1909, dominio público. Los nombres de los libros siguen el uso judío.'
            : 'Tanakh text in the World English Bible translation, public domain. Book names follow Jewish usage.'))
    : ($tradition === 'quran'
        ? ($locale === 'en' ? 'The Meaning of the Glorious Koran, English translation by Marmaduke Pickthall. Project Gutenberg eBook 16955; public domain in the USA.'
            : 'Quran arabe · texte Uthmani de QuranEnc · Quran JSON 3.1.2 · CC BY-SA 4.0.')
        : ($locale === 'fr' ? 'Bible Louis Segond 1910 via eBible.org, domaine public.'
            : ($locale === 'es' ? 'Biblia Reina-Valera 1909 via eBible.org, dominio público.' : 'World English Bible (WEBP), public domain.')));
...
$chapterUrl = static fn(array $unit): string => '?tradition='.rawurlencode($tradition).'&book='.rawurlencode($unit['book']).'&chapter='.$unit['chapter'].'&lang='.rawurlencode($locale).'#reader-top';
?>
<!doctype html>
<html lang="<?= e($locale) ?>" data-faith="<?= e($tradition) ?>" data-script-direction="<?= $tradition === 'quran' && $locale !== 'en' ? 'rtl' : 'ltr' ?>">
<head>
...
<style>.jaguar-cta{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:10px 14px;border:1px solid var(--accent);border-radius:999px;color:var(--accentInk);background:var(--accent);text-decoration:none;font:800 12px/1.2 Inter,system-ui,sans-serif;white-space:nowrap}.jaguar-cta:hover{filter:brightness(1.08);transform:translateY(-1px)}@media(max-width:760px){.jaguar-cta{width:100%;white-space:normal;text-align:center}}</style>
...
<?php foreach (['bible'=>'Bible','torah'=>'Tanakh','quran'=>'Quran'] as $key=>$label): ?><a class="<?= $tradition===$key?'active':'' ?>" href="?tradition=<?= e($key) ?>&lang=<?= e($locale) ?>"><?= e($label) ?></a><?php endforeach; ?>
</nav>
...
<script>
const tradition=<?= json_encode($tradition) ?>,book=<?= json_encode($book) ?>,locale=<?= json_encode($locale) ?>,bookSelect=document.getElementById('book-select'),chapterSelect=document.getElementById('chapter-select');bookSelect.onchange=()=>location.href='?tradition='+tradition+'&book='+encodeURIComponent(bookSelect.value)+'&chapter=1&lang='+locale+'#reader-top';if(chapterSelect)chapterSelect.onchange=()=>location.href='?tradition='+tradition+'&book='+encodeURIComponent(book)+'&chapter='+chapterSelect.value+'&lang='+locale+'#reader-top';

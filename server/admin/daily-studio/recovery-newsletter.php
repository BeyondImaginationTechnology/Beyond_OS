...
<style>
.background-picker{display:grid;gap:8px;margin:2px 0 4px;padding:0;border:0}.background-picker legend{margin-bottom:2px;color:var(--muted,#9ca3af);font-size:12px;font-weight:850;letter-spacing:.06em;text-transform:uppercase}.background-options{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.background-option{position:relative;display:flex!important;align-items:center;gap:8px;min-height:44px;padding:8px 9px;border:1px solid var(--border,#3a4050);border-radius:12px;background:#20232c;color:#edf0f5!important;font-size:11px!important;letter-spacing:0!important;text-transform:none!important;cursor:pointer}.background-option:has(input:checked){border-color:#e9be68;background:#2b2b2c;box-shadow:0 0 0 1px #e9be68 inset}.background-option input{position:absolute;opacity:0;inset:0;cursor:pointer}.background-option:focus-within{outline:2px solid #e9be68;outline-offset:2px}.background-swatch{flex:none;width:25px;height:25px;border:1px solid #ffffff4a;border-radius:8px}.background-swatch.forest{background:radial-gradient(circle at 80% 10%,#8b7041,#183d2d 55%,#071d14)}.background-swatch.sunrise{background:radial-gradient(circle at 70% 25%,#e59c69,#633e54 50%,#241a31)}.background-swatch.starlight{background:radial-gradient(circle at 25% 20%,#748ba9,#25364f 45%,#101a2e)}
</style>
...
<fieldset class="background-picker"><legend>Newsletter background</legend><div class="background-options" role="radiogroup" aria-label="Newsletter background"><label class="background-option"><input type="radio" name="newsletter-background" value="forest" checked><span class="background-swatch forest" aria-hidden="true"></span><span>Forest</span></label><label class="background-option"><input type="radio" name="newsletter-background" value="sunrise"><span class="background-swatch sunrise" aria-hidden="true"></span><span>Sunrise</span></label><label class="background-option"><input type="radio" name="newsletter-background" value="starlight"><span class="background-swatch starlight" aria-hidden="true"></span><span>Starlight</span></label></div></fieldset></div><div class="newsletter-actions"><button class="btn" id="export-pdf" type="button">Export PDF</button><button class="btn secondary" id="export-png" type="button">Export Instagram PNG</button></div><p class="export-note">PDF exports at US Letter proportions. Instagram PNG exports at exactly 1080 × 1080 pixels.</p></article>
...
const value=()=>({...seed,headline:field.headline.value,verse:field.verse.value,reference:field.reference.value,devotionalTitle:field['devotional-title'].value,devotional:field.devotional.value,challengeTitle:field['challenge-title'].value,challenge:field.challenge.value,background:document.querySelector('input[name="newsletter-background"]:checked').value});
...
function draw(canvas,mode){
  ...
  const themes={forest:{bg:'#071d14',glow:'rgba(208,163,76,.25)',glow2:'rgba(70,112,67,.12)',fade:'rgba(7,29,20,0)',border:'rgba(230,241,233,.16)',accent:'#eec96f',ink:'#f4f7f2',muted:'#c4d8ca',title:'#fffaf0',card:'rgba(255,255,255,.075)',challenge:'#24583f',challengeText:'#e4efe7',footer:'#b7d1bf'},sunrise:{bg:'#241a31',glow:'rgba(238,157,111,.28)',glow2:'rgba(157,92,111,.16)',fade:'rgba(36,26,49,0)',border:'rgba(255,232,220,.2)',accent:'#f2bc8e',ink:'#fff7f0',muted:'#e1c7c1',title:'#fff8f0',card:'rgba(255,245,238,.09)',challenge:'#633e54',challengeText:'#fff0e7',footer:'#e4c7c2'},starlight:{bg:'#101a2e',glow:'rgba(133,165,205,.28)',glow2:'rgba(70,103,151,.16)',fade:'rgba(16,26,46,0)',border:'rgba(222,234,255,.19)',accent:'#c3d3ee',ink:'#f0f5ff',muted:'#c0cfe3',title:'#f5f7ff',card:'rgba(230,239,255,.085)',challenge:'#253b5d',challengeText:'#e6efff',footer:'#becde4'}};
  const t=themes[d.background]||themes.forest;
  ctx.fillStyle=t.bg;ctx.fillRect(0,0,w,h);
  const glow=ctx.createRadialGradient(w*.83,h*.08,8,w*.83,h*.08,w*.72);glow.addColorStop(0,t.glow);glow.addColorStop(.55,t.glow2);glow.addColorStop(1,t.fade);ctx.fillStyle=glow;ctx.fillRect(0,0,w,h);
  if(d.background==='sunrise'){const sun=ctx.createRadialGradient(w*.12,h*.88,4,w*.12,h*.88,w*.48);sun.addColorStop(0,'rgba(242,188,142,.2)');sun.addColorStop(1,'rgba(242,188,142,0)');ctx.fillStyle=sun;ctx.fillRect(0,0,w,h)}
  if(d.background==='starlight'){ctx.fillStyle='rgba(224,236,255,.52)';for(let i=0;i<42;i++){const x=((i*173+61)%997)/997*w,y=((i*251+47)%1499)/1499*h,r=i%7===0?2:1;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()}}
  if(d.background==='forest'){ctx.fillStyle='rgba(139,185,145,.08)';for(let i=0;i<7;i++){ctx.beginPath();ctx.ellipse(w-(i*35+30),h*.22+i*27,90,190,-.5,0,Math.PI*2);ctx.fill()}}
  ctx.strokeStyle=t.border;ctx.lineWidth=2;ctx.beginPath();ctx.roundRect(22,22,w-44,h-44,square?28:34);ctx.stroke();
  const brandY=square?99:139;ctx.fillStyle=t.accent;ctx.beginPath();ctx.arc(m+9,brandY-8,square?8:10,0,Math.PI*2);ctx.fill();ctx.fillStyle=t.ink;ctx.font=`800 ${square?21:26}px Arial`;ctx.letterSpacing=square?'2px':'3px';ctx.fillText('DAILY BREATH',m+30,brandY);ctx.letterSpacing='0px';
  ctx.textAlign='right';ctx.fillStyle=t.muted;ctx.font=`700 ${square?15:19}px Arial`;ctx.fillText(String(d.issueDate).toUpperCase(),w-m,brandY);ctx.textAlign='left';
  ctx.fillStyle=t.title;ctx.font=`700 ${square?61:82}px Georgia`;ctx.letterSpacing=square?'-1px':'-1.5px';wrap(ctx,d.headline,m,square?184:246,content,square?68:91,2);ctx.letterSpacing='0px';
  const cardY=square?280:370,cardH=square?252:402,cardX=m,cardW=content;
  ctx.fillStyle=t.card;rounded(ctx,cardX,cardY,cardW,cardH,square?26:32);
  ctx.fillStyle=t.accent;rounded(ctx,cardX+23,cardY+25,5,cardH-50,3);
  ctx.fillStyle=t.accent;ctx.font=`800 ${square?16:21}px Arial`;ctx.letterSpacing='2px';ctx.fillText('VERSE FOR THE WEEK',cardX+48,cardY+(square?46:54));ctx.letterSpacing='0px';
  ctx.fillStyle=t.ink;ctx.font=`500 ${square?31:44}px Georgia`;wrap(ctx,'“'+d.verse+'”',cardX+48,cardY+(square?96:122),cardW-96,square?39:55,square?3:4);
  ctx.fillStyle=t.accent;ctx.font=`800 ${square?18:23}px Arial`;ctx.fillText(d.reference,cardX+48,cardY+cardH-(square?25:34));
  const reflectY=square?586:865;ctx.fillStyle=t.accent;ctx.fillRect(m,reflectY-15,square?34:42,4);ctx.fillStyle=t.accent;ctx.font=`800 ${square?15:20}px Arial`;ctx.letterSpacing='2px';ctx.fillText('RECOVERY REFLECTION',m+(square?48:58),reflectY);ctx.letterSpacing='0px';
  ctx.fillStyle=t.title;ctx.font=`700 ${square?31:40}px Arial`;const reflectionTitleLast=wrap(ctx,d.devotionalTitle,m,reflectY+(square?52:62),content,square?39:50,2);
  ctx.fillStyle=t.muted;ctx.font=`400 ${square?21:28}px Arial`;wrap(ctx,d.devotional,m,reflectionTitleLast+(square?40:52),content,square?29:39,square?2:3);
  const challengeY=square?772:1250,challengeH=square?205:282;ctx.fillStyle=t.challenge;rounded(ctx,m,challengeY,content,challengeH,square?24:30);
  ctx.fillStyle=t.accent;ctx.font=`800 ${square?15:20}px Arial`;ctx.letterSpacing='1.8px';ctx.fillText('THIS WEEK’S CHALLENGE',m+30,challengeY+(square?40:48));ctx.letterSpacing='0px';
  ctx.fillStyle=t.ink;ctx.font=`700 ${square?27:36}px Arial`;const challengeTitleLast=wrap(ctx,d.challengeTitle,m+30,challengeY+(square?80:98),content-60,square?34:44,2);
  ctx.fillStyle=t.challengeText;ctx.font=`400 ${square?19:25}px Arial`;wrap(ctx,d.challenge,m+30,challengeTitleLast+(square?31:43),content-60,square?25:33,square?2:3);
  ctx.fillStyle=t.footer;ctx.font=`700 ${square?13:17}px Arial`;ctx.letterSpacing=square?'1.4px':'2px';ctx.textAlign='center';ctx.fillText('ONE SMALL STEP · ONE STEADY BREATH · YOU ARE NOT ALONE',w/2,h-(square?34:52));ctx.textAlign='left';ctx.letterSpacing='0px';return canvas
}
...
ids.forEach(id=>field[id].addEventListener('input',refresh));document.querySelectorAll('input[name="newsletter-background"]').forEach(option=>option.addEventListener('change',refresh));

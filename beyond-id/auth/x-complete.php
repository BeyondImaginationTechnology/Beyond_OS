...
$pending = is_array($_SESSION['pending_x_identity'] ?? null) ? $_SESSION['pending_x_identity'] : [];
if ($pending === [] || time() - (int)($pending['created_at'] ?? 0) > 600 || empty($pending['subject'])) {
    unset($_SESSION['pending_x_identity']);
    $_SESSION['oauth_error'] = 'X sign-in expired. Please try again.';
    header('Location: login.php');
    exit;
}

$username = trim((string)($pending['username'] ?? ''));
$displayName = trim((string)($pending['name'] ?? '')) ?: ($username !== '' ? '@' . $username : 'X account');
$error = '';
$success = '';
    ...
    if (!verify_csrf_token($_POST['csrf'] ?? null)) {
        ...
        if (!$signedInUser || ($signedInUser['status'] ?? '') !== 'active') {
            ...
            try {
                ...
                if (!$linkedUserId) {
                    ...
                    $link->execute([(int)$signedInUser['id'], 'x', $pending['subject'], $signedInUser['email'], $displayName, $now, $now]);
                } else {
                    $update = $pdo->prepare('UPDATE social_identities SET email=?,display_name=?,updated_at=? WHERE provider=? AND provider_user_id=?');
                    $update->execute([$signedInUser['email'], $displayName, date('Y-m-d H:i:s'), 'x', $pending['subject']]);
                }
                $pdo->commit();
                log_activity($pdo, (int)$signedInUser['id'], 'oauth_link_x');
                beyond_rate_limit_clear($pdo, 'x-complete', (string)$pending['subject']);
                unset($_SESSION['pending_x_identity']);
                ...
                error_log('X account link failed: ' . $exception->getMessage());
                $error = $exception instanceof BeyondSocialUserException
                    ? $exception->getMessage()
                    : 'Beyond ID could not link this X account. Please try again.';
            }
        }
        ...
        elseif (empty($_POST['accept_terms'])) $error = 'Accept the Terms and Privacy Policy to create a Beyond ID.';
            ...
            try {
                ...
                $check->execute([$email]);
                if ($check->fetchColumn()) throw new BeyondSocialUserException('A Beyond ID already uses this email. Sign in to it above, then link X.');
                ...
                try { $pdo->prepare("UPDATE users SET terms_accepted_at=?,terms_version='2.1-beta-x' WHERE id=?")->execute([$now, $userId]); } catch (Throwable $exception) {}
                ...
                $link = $pdo->prepare('INSERT INTO social_identities (user_id,provider,provider_user_id,email,display_name,created_at,updated_at) VALUES (?,?,?,?,?,?,?)');
                $link->execute([$userId, 'x', $pending['subject'], $email, $displayName, $now, $now]);
                create_notification($pdo, $userId, 'Welcome to Beyond ID', 'Your Beyond ID is connected to X.', '/beyond-id/dashboard/profile.php', 'welcome');
                log_activity($pdo, $userId, 'register_x_terms_accepted_v2.1-beta');
                $pdo->commit();
                beyond_rate_limit_clear($pdo, 'x-complete', (string)$pending['subject']);
                unset($_SESSION['pending_x_identity']);
                send_beyond_id_admin_signup_alert(['id' => $userId, 'first_name' => $first, 'last_name' => $last, 'email' => $email, 'created_at' => $now], 'Beyond ID X signup');
                if ($isSqlite) {

...
function la_demo_access(): bool{return !empty($GLOBALS['academyConfig']['demo_access']);}
function la_full_access(): bool{$role=strtolower((string)($_SESSION['role']??''));return la_demo_access()||!empty($_SESSION['user_id'])||in_array($role,['admin','super_admin'],true)||!empty($_SESSION['learning_academy_entitled']);}
...
<?php if($laView==='home'): $modules=la_modules($academyConfig,$laAge);$dailyFactList=(array)($academyConfig['daily_history_facts']??[]);$dailyFact=$dailyFactList?end($dailyFactList):null;?><?php if(is_array($dailyFact)):?><section class="la-daily-fact"><div><span class="la-kicker">Daily History Fact · <span id="dailyHistoryNumber"><?=la_h((string)$dailyFact['number'])?></span>/55</span><h2 id="dailyHistoryTitle"><?=la_h((string)$dailyFact['title'])?></h2><p id="dailyHistoryFact"><?=la_h((string)$dailyFact['fact'])?></p><div class="la-daily-links"><a id="dailyHistorySource" href="<?=la_h((string)$dailyFact['source_url'])?>" target="_blank" rel="noopener">Open source ↗</a><a id="dailyHistoryInstagram" href="<?=la_h((string)$dailyFact['instagram_url'])?>" target="_blank" rel="noopener">Instagram reference ↗</a></div></div><aside><strong>Today’s lesson</strong><p id="dailyHistoryLesson"><?=la_h((string)$dailyFact['lesson'])?></p><a class="la-button" href="?view=lesson&amp;age=<?=la_h($laAge)?>&amp;module=archaeology-historical-evidence&amp;lesson=1">Open evidence lesson →</a></aside></section><?php endif;?>

...
<title>Create or reset your password | Beyond ID</title>
...
<body><main class="card"><h1>Create or reset your password</h1><p>Enter the email on your Beyond ID. If you use Google, GitHub, or Apple, keep using that provider unless you want to add password sign-in too. We’ll email a one-hour reset link.</p><?php if ($message): ?><div class="msg" role="status" aria-live="polite"><?= e($message) ?></div><?php endif; ?><?php if ($error): ?><div class="err" role="alert"><?= e($error) ?></div><?php endif; ?><form method="post"><input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>"><p><label for="email">Email address</label></p><input id="email" type="email" name="email" value="<?= e($email) ?>" autocomplete="email" required><button type="submit">Email me a password link</button></form><p><a class="app-back" href="login.php">← Back to sign in</a></p></main><script src="/assets/js/visitor-analytics.js" defer></script></body>
</html>

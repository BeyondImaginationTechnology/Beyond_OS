...
$channelNumber = str_pad((string)($channel['display_number'] ?? '1'), 2, '0', STR_PAD_LEFT);
$channelArt = [
...
</style></head><body class="tv-app channel-view" style="--channel-art:url('/beyond-tv/assets/img/<?=htmlspecialchars($activeChannelArt[0])?>');--channel-art-position:<?=htmlspecialchars($activeChannelArt[1])?>;--channel-gradient:<?=htmlspecialchars((string)($channel['gradient'] ?? 'linear-gradient(135deg,#511a62,#15215f)'))?>">
...
<?php elseif(($isScheduled || $isSpaceScheduled) && $scheduledState): ?><div class="youtube-player-wrap"><iframe class="youtube-player" src="<?=htmlspecialchars((string)$scheduledState['embed_url'])?>" title="<?=htmlspecialchars((string)$channel['name'])?> synchronized live channel" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div><div class="playlist-actions"><span class="source-pill">LIVE · VANCOUVER TIME</span><span><?=htmlspecialchars((string)($scheduledState['playing']['title']??$scheduledState['current']['title']))?> · joins in progress</span></div>
...
<div class="channel-station-line"><span class="live-badge">LIVE</span><span>BEYOND TV · CHANNEL <?=$channelNumber?></span><span class="signal-bars"><i></i><i></i><i></i><i></i></span></div>
...
<div class="channel-broadcast-meta"><span>● ON AIR</span><span>VANCOUVER TIME</span><span>FREE TO WATCH</span></div>

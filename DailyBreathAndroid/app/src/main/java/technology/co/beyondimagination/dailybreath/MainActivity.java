...
public final class MainActivity extends Activity {
    ...
    private void showJournal(){title("REFLECTION JOURNAL","Private space for the next honest thought");EditText input=new EditText(this);input.setHint("What is present for you today?");input.setGravity(Gravity.TOP);input.setMinLines(5);input.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_FLAG_MULTI_LINE|InputType.TYPE_TEXT_FLAG_CAP_SENTENCES);input.setContentDescription("Journal reflection");page.addView(input,spaced());Button save=action("Save reflection on this device");save.setOnClickListener(v->saveJournal(input));page.addView(save,spaced());TextView heading=label("Saved reflections",20,INK,true);heading.setPadding(0,dp(26),0,dp(8));page.addView(heading);renderJournal();}
    private void saveJournal(EditText input){String text=input.getText().toString().trim();if(text.isEmpty()){input.setError("Write a reflection before saving.");return;}try{JSONArray old=new JSONArray(prefs.getString("journal_entries","[]")),next=new JSONArray();next.put(new JSONObject().put("date",LocalDate.now().toString()).put("text",text));for(int i=0;i<Math.min(49,old.length());i++)next.put(old.getJSONObject(i));prefs.edit().putString("journal_entries",next.toString()).apply();Toast.makeText(this,"Reflection saved.",Toast.LENGTH_SHORT).show();showTab(4);}catch(Exception error){Toast.makeText(this,"Reflection could not be saved.",Toast.LENGTH_LONG).show();}}
    private void renderJournal(){try{JSONArray entries=new JSONArray(prefs.getString("journal_entries","[]"));if(entries.length()==0){addBody("No saved reflections yet.");return;}for(int i=0;i<entries.length();i++){JSONObject entry=entries.getJSONObject(i);LinearLayout item=card(Color.WHITE);item.addView(label(entry.optString("date"),12,PURPLE,true));item.addView(label(entry.optString("text"),15,Color.DKGRAY,false));page.addView(item,spaced());}}catch(Exception error){addBody("Saved reflections could not be opened.");}}

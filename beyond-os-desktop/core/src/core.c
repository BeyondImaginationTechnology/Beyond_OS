...
#define FOUNDATION_NOTE "Lean independent Linux foundation"
#define UPCOMING_NOTE "Optional apps, hardware support and signed updates are next milestones."
static const char *titles[] = {"Files", "Notes", "Terminal", "About Core"};
...
static bool save_note(void)
{
    ...
    dirty = false;
    snprintf(status, sizeof status, "Saved to Documents/Core Note.txt");
    return true;
}
...
static void activate(int card)
{
    ...
    else if (card == 1) { if (!note_writable) { snprintf(status, sizeof status, "Notes could not load the existing file (unreadable or over 8 KB). It has not been changed."); return; } page = NOTES; SDL_StartTextInput(); snprintf(status, sizeof status, "Type a note. Ctrl+S saves. Core also saves before leaving."); }
...
static void draw(void)
{
    ...
    if (page == HOME) {
        ...
        text(font, "Core", 72, 99, white);
        const char *heading = page == FILES ? "Files" : page == NOTES ? "Notes" : page == VIEWER ? file_title : "About Core";
        ...
        if (page == FILES) {
            ...
            if (page == NOTES) {
                text(small_font, dirty ? "Unsaved changes  /  Ctrl+S to save" : "Documents/Core Note.txt", 51, 704, accent);
            }
        } else {
#ifdef BIT_EDITION_CYBER
...
int main(int argc, char **argv)
{
    ...
    if (!join_path(note_path, sizeof note_path, home, "Documents/Core Note.txt") ||

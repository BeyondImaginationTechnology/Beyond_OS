...
$bookName = $tradition === 'quran' ? ($surahs[(int)$book] ?? ('Surah '.$book)) : $book;
$readerTitle = $tradition === 'quran' ? 'Surah '.$bookName.' · '.$book : $bookName.' '.$chapter;
$translation = $tradition === 'torah'
    ? 'World English Bible — Hebrew Scriptures, public domain.'
    : ($tradition === 'quran'
...
<form class="search" method="get"><input type="hidden" name="tradition" value="<?= e($tradition) ?>"><input name="q" value="<?= e($query) ?>" placeholder="Search all <?= e($traditionLabel) ?> verses" aria-label="Search <?= e($traditionLabel) ?>"><button>Search</button></form>
...
<div class="library">
<aside class="books"><?php foreach ($groups as $section=>$items): ?><h2><?= e($section) ?></h2><div class="book-grid"><?php foreach ($items as $itemBook=>$count): ?><?php $itemName=$tradition==='quran' ? $itemBook.'. '.($surahs[(int)$itemBook]??'Surah') : $itemBook; ?><a class="<?= $book===(string)$itemBook?'active':'' ?>" href="?tradition=<?= e($tradition) ?>&book=<?= rawurlencode((string)$itemBook) ?>&chapter=1#reader-top"><?= e($itemName) ?></a><?php endforeach; ?></div><?php endforeach; ?></aside>
<article class="reader" id="reader-top">
<div class="reader-head"><div><span class="eyebrow"><?= e(strtoupper($traditionLabel)) ?> · COMPLETE LOCAL EDITION</span><h1><?= e($readerTitle) ?></h1></div><button class="narrate" id="narrate" type="button">▶ Narrate</button></div>
<div class="chapter-controls"><div class="pickers"><label><?= $tradition==='quran'?'Surah':'Book' ?><select id="book-select"><?php foreach ($groups as $section=>$items): ?><optgroup label="<?= e($section) ?>"><?php foreach ($items as $itemBook=>$count): ?><?php $itemName=$tradition==='quran' ? $itemBook.'. '.($surahs[(int)$itemBook]??'Surah') : $itemBook; ?><option value="<?= e((string)$itemBook) ?>" <?= $book===(string)$itemBook?'selected':'' ?>><?= e($itemName) ?></option><?php endforeach; ?></optgroup><?php endforeach; ?></select></label><?php if ($tradition!=='quran'): ?><label>Chapter<select id="chapter-select"><?php for($number=1;$number<=$flatBooks[$book];$number++): ?><option value="<?= $number ?>" <?= $number===$chapter?'selected':'' ?>><?= $number ?></option><?php endfor; ?></select></label><?php endif; ?></div><?php if ($tradition!=='quran'): ?><nav class="chapters" aria-label="Chapters"><?php for($number=1;$number<=$flatBooks[$book];$number++): ?><a class="<?= $number===$chapter?'active':'' ?>" href="?tradition=<?= e($tradition) ?>&book=<?= rawurlencode($book) ?>&chapter=<?= $number ?>#reader-top"><?= $number ?></a><?php endfor; ?></nav><?php endif; ?></div>
...
<nav class="bottom"><a href="index.php?faith=<?= e($tradition) ?>">Home</a><a href="devotionals.php">Devotionals</a><a class="active" href="scripture.php?tradition=<?= e($tradition) ?>">Scripture</a><a href="academy.php">Academy</a><a href="practices.php?section=breathing">Breathe</a></nav>
<script>
...
const narrate=document.getElementById('narrate'),chapterText=document.getElementById('chapter-text');let speaking=false;narrate.onclick=()=>{if(!('speechSynthesis'in window))return;if(speaking){speechSynthesis.cancel();return}const utterance=new SpeechSynthesisUtterance([...chapterText.querySelectorAll('.verse')].map(v=>v.innerText).join(' '));utterance.rate=Number(window.DailyBreath?.settings?.narrationRate||.92);utterance.onstart=()=>{speaking=true;narrate.textContent='■ Stop'};utterance.onend=utterance.onerror=()=>{speaking=false;narrate.textContent='▶ Narrate'};speechSynthesis.speak(utterance)};

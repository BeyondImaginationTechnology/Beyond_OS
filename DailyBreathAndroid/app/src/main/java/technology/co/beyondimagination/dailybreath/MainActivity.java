...
public final class MainActivity extends Activity {
    ...
    private void addFaithPicker() {
        ...
        for(Faith candidate:Faith.values()){ Button button=new Button(this); button.setText(candidate.title); button.setTextSize(13); button.setAllCaps(false); button.setTypeface(Typeface.DEFAULT,Typeface.BOLD); boolean selected=candidate==faith; button.setTextColor(selected?Color.WHITE:INK); button.setBackground(round(selected?FOREST:Color.TRANSPARENT,12)); button.setContentDescription("Choose "+candidate.title); button.setOnClickListener(v->{faith=candidate;prefs.edit().putString("selected_faith",faith.name()).apply();showTab(tab);}); picker.addView(button,new LinearLayout.LayoutParams(0,dp(46),1)); }
    ...
    private Button action(String text){Button button=new Button(this);button.setText(text);button.setAllCaps(false);button.setTextSize(15);button.setTextColor(Color.WHITE);button.setTypeface(Typeface.DEFAULT,Typeface.BOLD);button.setBackground(round(FOREST,16));button.setPadding(dp(14),dp(11),dp(14),dp(11));button.setContentDescription(text);return button;}

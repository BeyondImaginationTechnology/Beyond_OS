...
<script>
const button=document.getElementById('narrate'),text=document.getElementById('chapter-text');let speaking=false;button.onclick=()=>{if(!('speechSynthesis'in window))return;if(speaking){speechSynthesis.cancel();return}const narrationText=[...text.querySelectorAll('.verse')].map(verse=>verse.innerText).join(' ');const utterance=new SpeechSynthesisUtterance(narrationText);utterance.rate=Number(window.DailyBreath?.settings?.narrationRate||.92);utterance.onstart=()=>{speaking=true;button.textContent='■ Stop narration'};utterance.onend=utterance.onerror=()=>{speaking=false;button.textContent='▶ Narrate chapter'};speechSynthesis.speak(utterance)};

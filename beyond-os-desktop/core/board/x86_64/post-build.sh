...
PRETTY_NAME="BIT OS Core v.02 (Development)"
ID=beyond-os
...
printf '%s\n' 'Welcome to BIT OS Core v.02.' 'This VM development image is built from upstream Linux components.' > "$target/home/home/Documents/Welcome.txt"
printf '%s\n' 'BIT OS Core v.02 (Development)' > "$target/etc/issue"
printf '%s\n' 'BIT OS Core v.02 (Development)' > "$target/etc/motd"
